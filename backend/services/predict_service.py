...
class PredictService:
    ...
    @classmethod
    def load_models(cls):
        """Loads machine learning models (RoBERTa or TF-IDF baseline) or sets up demo fallback."""
        t0 = time.time()
        
        # Try RoBERTa first
        if _HAS_TORCH and os.path.isdir(MODEL_DIR) and os.path.exists(os.path.join(MODEL_DIR, "config.json")):
            try:
                ...
                log.info(f"[SUCCESS] RoBERTa loaded (device={'cuda' if device == 0 else 'cpu'})")
        ...
        if os.path.exists(TFIDF_PATH) and os.path.exists(LR_PATH):
            try:
                ...
                if not cls._model_loaded:
                    cls._model_type = "tfidf_lr"
                    cls._model_loaded = True
                log.info("[SUCCESS] TF-IDF + LR baseline loaded")
            except Exception as e:
                log.warning(f"[WARNING] Baseline load failed: {e}")

        if not cls._model_loaded:
            log.warning("[WARNING] No models found — running in demo mode (random predictions)")
    ...
    def get_model_info(cls) -> dict:
        return {
            ...
            "base_model": "roberta-base" if cls._model_type == "roberta" else "tfidf_lr" if cls._model_type == "tfidf_lr" else "demo_mode",
            "max_length": 384,
            "labels": ["FAKE", "REAL"],
            "roberta_available": cls._roberta_pipe is not None,
            "baseline_available": cls._tfidf is not None,
    ...
    def _predict_one(cls, text: str) -> dict:
        ...
        elif cls._model_type in ("tfidf_lr", "roberta") and cls._tfidf is not None:
            try:
                X = cls._tfidf.transform([text])
                prob = cls._lr_model.predict_proba(X)[0]
                raw_prob_fake = float(prob[0])
                raw_prob_real = float(prob[1])
                model_used = "tfidf_lr_baseline"
            except Exception as e:
                log.error(f"TF-IDF Baseline prediction error: {e}")
                
        if cls._model_type == "demo" or (cls._model_type != "roberta" and cls._tfidf is None):
            # Demo mode / Fallback
            h = hash(text) % 100
            is_fake = h < 50
            conf = 0.55 + (h % 30) / 100
